...
const EmailModal = ({ open, onOpenChange, onSubmit, defaultEmail }: { open: boolean, onOpenChange: (open: boolean) => void, onSubmit: (data: any) => void, defaultEmail?: string }) => {
    ...
    <div className="container mx-auto py-4 max-w-6xl">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span>{order.eventDate ? format(new Date(order.eventDate), 'EEE, dd MMM yyyy') : 'Tue, 06 May 2025'}</span>
            </div>
            <div className="flex justify-between">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span>{order.deliveryType === 'Delivery' ? 'Delivered to Kindy' : 'Pickup'}</span>
            </div>
            <div className="mt-4">
      ...
      <div className="bg-white rounded-lg shadow-sm mt-6">
        ...
        <div className="w-full">
          ...
          {(order.items?.length ? order.items : [
            {id: 1, name: 'Chocolate Butterscotch Slice', description: '20 serves', quantity: 1, price: '30.00'},
            {id: 2, name: 'Lemon Brownie', description: '15 serves', quantity: 1, price: '30.00'}
          ]).map((item: any, index: number) => (
            <div key={item.id || index} className="border-t border-gray-100 p-4">
              <div className="flex justify-between">
                <div>
                  <div className="text-blue-500 mb-1">Item</div>
                  <div>{item.quantity || 1} x {item.name}</div>
                  <div className="text-sm text-gray-500">Details: {item.description}</div>
                </div>
                <div className="text-right">
                  <div className="mb-4">$ {parseFloat(item.price).toFixed(2)}</div>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm mt-6">
        ...
        <div className="min-h-[80px]">
          <p className="text-sm text-gray-600">{order.notes || 'No additional notes for this order.'}</p>
        </div>
      </div>
          ...
          {order.tasks?.length > 0 ? (
              ...
              {order.tasks.map((task: any) => (
                <div key={task.id} className="flex items-center justify-between py-2 border-b border-gray-100">
                  <div className="flex items-center">
                    ...
                    />
                    <label htmlFor={`task-${task.id}`} className={`text-sm ${task.completed ? 'line-through text-gray-400' : ''}`}>
                      {task.description}
                    </label>
                  </div>
            ...
            <div className="flex items-center justify-center py-2">
              <div className="flex items-center">
                ...
                <label htmlFor="task342542" className="text-sm">Check cake specifications</label>
              </div>
            ...
            {order.logs?.length > 0 ? (
              order.logs.map((log: any) => (
                <tr key={log.id} className="border-b border-gray-100">
                  <td className="py-3 text-sm">{format(new Date(log.created_at), 'EEE, dd MMM yyyy - hh:mma')}</td>
                  <td className="py-3 text-sm">
                    {log.type === 'payment' ? (
                      <>
                        {log.action} - ${log.amount} - 
                        <a href="#" className="text-blue-500 ml-1">Payment Receipt</a>
                      </>
                    ) : log.type === 'email' ? (
