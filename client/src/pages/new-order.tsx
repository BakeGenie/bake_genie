...
const NewOrderPage = () => {
  ...
  const preselectedDate =
    searchParams.get("eventDate") || searchParams.get("date");
  ...
  const handleNewOrderSubmit = async (data: OrderFormValues) => {
    try {
      ...
      console.log(
        "STEP 1 - Raw form data received:",
        JSON.stringify(data, null, 2),
      );

      // Format the form data for the API request to match what the server expects
      // NOTE: The column names in the database must exactly match these field names 
      // Based on the orders-direct.ts file that handles the actual SQL insert
      const formattedData = {
        ...
        // The database column is actually named "total_amount" not "total"
        total: totalAmount(
          data.items,
          data.discount || 0,
          data.setupFee || 0,
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader title="New Order" backLink="/orders" backLabel="Orders" />
